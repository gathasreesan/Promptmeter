...
The optimizer has a dependency-free regression suite (179 checks) that verifies protected spans come back
...
### 1b. Machine Learning Component

A lightweight classifier that closes the blind spot in a pure blacklist: a sentence nobody
wrote a rule for. Trained on **512 labeled phrases**.

| Metric | Four-class | KEEP vs DROP |
| :--- | ---: | ---: |
| Accuracy | 0.711 | **0.906** |
| Precision (macro) | 0.710 | 0.876 |
| Recall (macro) | 0.708 | 0.889 |
| F1-score (macro) | 0.707 | 0.882 |

5-fold CV accuracy **0.779 (± 0.053)**. KEEP vs DROP is the call the optimizer actually
consumes — the three removable labels collapse into one decision.

The whole model exports to **86 KB** of vocabulary, IDF weights and coefficients, so inference
is one sparse dot product in JavaScript: no server, no WASM, and prompt text never leaves the
browser. Setting both thresholds above 1.0 disables the assist and restores pure rule-based
behaviour, which the test suite verifies.

```
Prompt → protect → rule-based analysis → ML classification → combined decision
